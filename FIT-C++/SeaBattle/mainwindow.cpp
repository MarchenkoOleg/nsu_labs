    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->tableWidget_myField->setEnabled(false);
    ui->tableWidget_myField->setEnabled(true);
    for(int i = 0; i<10; i++){
        for (int j = 0; j<10; j++){
    ...
    ui->pushButton_place_ships->setText(tr("Начать игру"));
    ui->pushButton_clear->setText(tr("Очистить поле"));
    ui->label_1->setText(tr("Вы"));
    ui->label_2->setText(tr("Противник"));
...
void MainWindow::on_actionsave_game_triggered()
{
    ...
    QString filename = QFileDialog::getSaveFileName(this, tr("Сохранить текущую игру"),
        QDir::homePath() + "/SeaBattle/saved games", tr("Game files *.sb (*.sb)"));
...
void MainWindow::reprintField(const std::string & key)
{
    ...
    if (key == "my"){
        ...
        for (int i = 0; i < 10; i++) {
            for (int j = 0; j < 10; j++) {
                if (currentField[10 * i + j] == Cell::CL_FULL
                        || currentField[10 * i + j] == Cell::CL_HALF){
                    ui->tableWidget_myField->item(i,j)->setBackgroundColor(Qt::red);
...
void MainWindow::on_tableWidget_myField_cellDoubleClicked(int row, int column)
{
    return;
    if (!(ui->tableWidget_myField->item(row,column)->flags() == Qt::ItemIsDragEnabled)) {
        if (ui->tableWidget_myField->item(row,column)->backgroundColor() == Qt::white) {
            ui->tableWidget_myField->item(row, column)->setBackground(Qt::red);
        }
    }
}
...
void MainWindow::on_tableWidget_myField_cellClicked(int row, int column)
{
    ...
    //if (!(ui->tableWidget_myField->item(row,column)->flags() == Qt::ItemIsDragEnabled)) {
        if (ui->tableWidget_myField->item(row,column)->backgroundColor() == Qt::red) {
...
void MainWindow::checkPlacing()
{
    ...
    for (int i = 0; i < 10; i++){
        for (int j = 0; j < 10; j++){
            currentField[10 * i + j] = Cell::CL_CLEAR;
            if (ui->tableWidget_myField->item(i,j)->backgroundColor() == Qt::red){
                currentField[10 * i + j] = Cell::CL_SHIP;
            }
        }
    }
...
{
    ShootResult contin = enemyField.shoot(y, x);
    reprintField("enemy");
...
void MainWindow::enemyAttack()
{
    ...
    if (shootedPoints != tmp) {
        decision = comp.makeReasonDecision(myField.getFieldInstanceForEnemy(), shootedPoints);
    } else {
        decision = comp.makeRawDecision(myField.getFieldInstanceForEnemy());
    }
    ...
    if (contin == ShootResult::WOUND) shootedPoints[10 * decision.second + decision.first] = Cell::CL_HALF;

    while(contin != ShootResult::NO_HITS){
        if (myField.getNumOfShips() == 0){
            seaBattle.status = GameStatus::EndOfGame;
            endGame("Противник победил!");
            return;
        }
        if (contin == ShootResult::WOUND) { // (ранил)
            decision = comp.makeReasonDecision(myField.getFieldInstanceForEnemy(), shootedPoints);
            contin = myField.shoot(decision.first, decision.second);
            QTest::qWait(1000);
            reprintField("my");
            ...
            if (contin == ShootResult::WOUND) {
                shootedPoints[10 * decision.second + decision.first] = Cell::CL_HALF;
            }
        }
